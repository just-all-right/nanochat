...
| **个人 GPU 配额** | ⚠️ **最多 2 张**（QoS `gpu2limit` 限制） |
| **CPU** | 112 核 |
| **内存** | ~386 GB |
| **CUDA 驱动** | 13.2 |
| **Slurm 版本** | 25.05.0 |
...
| QoS | `gpu2limit`（限制每用户最多使用 **2 张** GPU） |
| 公平份额权重 | 1（`RawShares`） |
...
| `--gres` | 通用资源 | `gpu:1` 到 `gpu:2`（受 QoS 限制） |
| `--time` / `-t` | 时间限制 | 最大 `1-00:00:00`（1 天） |
| `--ntasks` / `-n` | 任务数 | 通常为 1（单个 Python 进程） |
| `--cpus-per-task` / `-c` | 每任务的 CPU 数 | 建议 8-16（2 GPU 时 ≤ 32） |
| `--mem` | 内存 | 可选，如 `--mem=64G` |
...
#SBATCH --gres=gpu:2               # 最多 2 张（QoS 限制）
...
本集群只有 1 个节点，你最多可申请 2 张 GPU。推荐在单个 Slurm 任务中申请 2 张 GPU，然后通过 `torchrun` 启动 2 进程 DDP：
...
echo "Starting DDP training on ${SLURM_GPUS_ON_NODE:-2} GPUs..."
...
torchrun --standalone --nproc_per_node=2 \
    -m scripts.base_train \
    --depth=6 --max-seq-len=256 \
    --device-batch-size=4 --num-iterations=50 \
    --run=ddp_test
...
| 四（预训练-标准） | `base_train.py --depth=20` | 2 GPU (DDP) | `sbatch` | ~4-6 小时 |
| 五（优化器） | 代码走读 + 小测试 | 1 GPU | `srun --pty` | 4 小时 |
| 六（SFT） | `chat_sft.py` | 2 GPU (DDP) | `sbatch` | ~2-4 小时 |
| 七（RL） | `chat_rl.py` | 2 GPU (DDP) | `sbatch` | ~4-6 小时 |
| 八（推理引擎） | `chat_web.py` / `chat_cli.py` | 1 GPU | `srun --pty` | 3 小时 |
| 九（评估） | `base_eval.py` / `chat_eval.py` | 1 GPU | `srun` | 1-2 小时 |
| 十（全流程） | 整合上述 | 2 GPU | `sbatch` | ~8-12 小时 |
...
```bash
# 减小设备批次大小
--device-batch-size=16  # 默认 32，可以降到 16、8、4

# 减小上下文长度
--max-seq-len=1024  # 默认 2048
...
#SBATCH --gres=gpu:2               # 2 张 GPU（QoS 限制）
...
echo "Job ${SLURM_JOB_ID}: base_train depth=20 on 2 GPUs"
